...
<div class="container mx-auto py-10 pt-20" id="compare-container">
    ...
    @if($products->count() >= 1)
    <div class="overflow-auto bg-white rounded-xl shadow-xl ring-1 ring-gray-200" id="compare-table">
            ...
            <thead class="sticky top-0 z-10 bg-blue-50 text-xs uppercase text-gray-600">
                <tr>
                    ...
                    @foreach ($products as $p)
                        <th class="p-4 text-center relative group align-top min-w-[220px] bg-white" data-id="{{ $p->id }}">
            ...
            <tbody class="text-sm">
                ...
                @foreach ($rows as $label => $callback)
                    <tr class="border-t hover:bg-gray-50 transition">
                        ...
                        @foreach ($products as $p)
                            <td class="p-4 text-center align-middle" data-id="{{ $p->id }}">{!! $callback($p) !!}</td>
                        @endforeach
                    </tr>
                @endforeach
            </tbody>
...
function removeFromCompare(productId) {
    ...
    // Xoá tất cả cột & cell có data-id = productId
    document.querySelectorAll(`[data-id="${productId}"]`).forEach(el => el.remove());

    // Nếu chỉ còn 1 sản phẩm → remove luôn cột thuộc tính còn lại
    const remaining = document.querySelectorAll('thead th').length - 1;
    if (remaining === 0) {
